...
cv_df = 
  ...
  mutate(
    ...
    model_2 = map(train, \(df) lm(bwt ~ blength + gaweeks, data = birthwt_df)),
    model_3 = map(train, \(df) lm(bwt ~ bhead* babysex + blength* babysex + bhead* blength* babysex, data = birthwt_df))
  ) |> 
   mutate(
